...
class Project(models.Model):
    ...
    technician = models.ForeignKey('Technician', on_delete=models.SET_NULL, null=True, related_name='projects')
    ...
    coordination_board = models.ImageField(upload_to=coordination_board_path, blank=True, null=True)
    ...
    versions_matrix = models.TextField(blank=True, help_text="Collez ici le tableau des versions (Markdown/CSV/texte).")
    ...
    start_at = models.DateTimeField(null=True, blank=True, help_text="Date et heure de début du projet")
    end_at   = models.DateTimeField(null=True, blank=True, help_text="Date et heure de fin du projet")
    ...
    def phases_completed(self) -> int:
        return sum(
            1 for s in [self.preparation_phase, self.execution_phase, self.validation_phase]
            if s == 'completed'
        )
...
class CoordinationCreateForm(forms.Form):
    project_number = forms.CharField(
        ...
        max_length=50,
        widget=forms.TextInput(attrs={"class": "form-control", "placeholder": "PRJ12345"})
    )
    ...
    prep_date = forms.DateField(
        label="Date (préparation)", required=False,
        widget=forms.DateInput(attrs={"type": "date", "class": "form-control"})
    )
    prep_start_time = forms.TimeField(
        label="Heure début (préparation)", required=False,
        widget=forms.TimeInput(attrs={"type": "time", "class": "form-control"})
    )
    prep_end_time = forms.TimeField(
        label="Heure fin (préparation)", required=False,
        widget=forms.TimeInput(attrs={"type": "time", "class": "form-control"})
    )

    # Production
    prod_date = forms.DateField(
        label="Date (production)", required=False,
        widget=forms.DateInput(attrs={"type": "date", "class": "form-control"})
    )
    prod_start_time = forms.TimeField(
        label="Heure début (production)", required=False,
        widget=forms.TimeInput(attrs={"type": "time", "class": "form-control"})
    )
    prod_end_time = forms.TimeField(
        label="Heure fin (production)", required=False,
        widget=forms.TimeInput(attrs={"type": "time", "class": "form-control"})
    )
