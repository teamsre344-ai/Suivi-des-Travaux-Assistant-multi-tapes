...
FALLBACK_PRODUCTS = [
    "Dossier patient",
    "Planification blocs opératoires",
    "RIS/PACS",
    "Gestion des lits",
    "Portail cliniciens",
    "Portail patients",
    "Facturation",
    "Interfaçage HL7/FHIR",
    "BI/Analytique",
    "Pharmacie",
]
...
def pick_from_choices(
    field: models.Field, preferred: list[str] | None = None
) -> Optional[str]:
...
def ensure_client_object(client_fk: models.ForeignKey) -> models.Model:
    ...
    for f in ClientModel._meta.fields:
        if isinstance(f, models.CharField) and f.name in (
            "name",
            "title",
            "label",
            "display_name",
    ...
    try:
        ...
        cf = next(
            (f for f in ClientModel._meta.fields if isinstance(f, models.CharField)),
            None,
        )
        if cf:
...
def get_seed_user() -> models.Model:
    ...
    return User.objects.get_or_create(
        username="seed.bot",
        defaults={
            "email": "seed.bot@lgisolutions.com",
            "password": "unused-seed-password",
        },
    )[0]
...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        parser.add_argument(
            "--count", type=int, default=25, help="How many projects to create."
        )
        parser.add_argument(
            "--start",
            type=int,
    ...
    def handle(self, *args, **opts):
        ...
        project_number_field = (
            "project_number" if "project_number" in proj_fields else None
        )
        if not project_number_field:
            raise SystemExit(
                "This command requires a 'project_number' field on Project."
            )

        # common fields (set only if present)
        title_field = (
            "title"
            if "title" in proj_fields
            else ("name" if "name" in proj_fields else None)
        )
        description_field = "description" if "description" in proj_fields else None
        start_field = (
            "start_date"
            if "start_date" in proj_fields
            else ("date_start" if "date_start" in proj_fields else None)
        )
        due_field = (
            "due_date"
            if "due_date" in proj_fields
            else ("end_date" if "end_date" in proj_fields else None)
        )
        budget_field = "budget" if "budget" in proj_fields else None
        progress_field = (
            "progress"
            if "progress" in proj_fields
            else ("progress_percent" if "progress_percent" in proj_fields else None)
        )
        ...
        product_fk = (
            proj_fields[product_field_name]
            if product_field_name
            and isinstance(proj_fields[product_field_name], models.ForeignKey)
            else None
        )
        ...
        for cand in ("client", "customer"):
            if cand in proj_fields:
                if isinstance(proj_fields[cand], models.ForeignKey):
                    ...
                    proj_fields[cand], (models.CharField, models.TextField)
                ):
        ...
        if not client_fk_field and not client_char_field:
            ...
            for cand in ("client_name", "customer_name", "hospital_name"):
                if cand in proj_fields and isinstance(
                    proj_fields[cand], (models.CharField, models.TextField)
                ):
                    client_char_field = cand
                    break
        ...
        technician_field = (
            "technician"
            if "technician" in proj_fields
            and isinstance(proj_fields["technician"], models.ForeignKey)
            else None
        )
        ...
        while created_count < target_count:
            ...
            if description_field:
                data[description_field] = (
                    f"Déploiement du produit « {prod_name} » à {hospital}. (Données de démonstration)"
                )
            ...
            if environment_field:
                env_val = pick_from_choices(
                    proj_fields[environment_field], ["PROD", "TEST", "QA"]
                )
            ...
            if product_field_name:
                if product_fk:
                    ...
                    val = (
                        pick_from_choices(proj_fields[product_field_name], [prod_name])
                        or prod_name
                    )
